...
start "Hermes API" cmd /c "cd /d "%UI_DIR%" && python -m uvicorn api_server.main:app --port 8080 --host 0.0.0.0"
...
start "XAUUSD Dashboard" cmd /c "cd /d "%BOT_DIR%" && python -m streamlit run src/daily_xauusd_brief/dashboard.py --server.port 8501 --server.address 127.0.0.1 --server.headless true --server.enableCORS true --server.enableXsrfProtection false"
echo   Dashboard: http://localhost:8501  ^(internal only^)
echo.

REM ── 4. Web UI — serve dist (5173) ──────────────────────────────────────────
echo [4/3] Starting Web UI on :5173...
start "Hermes Web UI" cmd /c "cd /d "%UI_DIR%" && npx --yes serve dist -l 5173 -s"
